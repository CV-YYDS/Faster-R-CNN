...
class RoI(Function):
    ...
    def forward(self, x, rois):
        ...
        #output = t.zeros(N, C, self.outh, self.outw).to(cuda0)
        self.argmax_data = t.zeros(N, C, self.outh, self.outw).int().cuda()
        #self.argmax_data = t.zeros(N, C, self.outh, self.outw).int().to(cuda0)
        self.rois = rois
    ...
    def backward(self, grad_output):
        ...
        #grad_input = t.zeros(self.in_size).to(cuda0)
        stream = Stream(ptr=torch.cuda.current_stream().cuda_stream)
...
def test_roi_module():
    ...
    #bottom_data = t.randn(B, C, H, W).to(cuda0)
    ...
    #bottom_rois = bottom_rois.to(cuda0)
    spatial_scale = 1. / 16
    outh, outw = PH, PW
